...
import com.aliyuncs.v5.vod.model.v20170321.ListDNADBResponse;
import com.aliyuncs.v5.vod.model.v20170321.ListDNADBResponse.DNADB;
...
public class ListDNADBResponseUnmarshaller {
	...
	public static ListDNADBResponse unmarshall(ListDNADBResponse listDNADBResponse, UnmarshallerContext _ctx) {
		...
		List<DNADB> dNADBListList = new ArrayList<DNADB>();
		for (int i = 0; i < _ctx.lengthValue("ListDNADBResponse.DNADBList.Length"); i++) {
			...
			dNADBListList.add(dNADB);
		}
		listDNADBResponse.setDNADBList(dNADBListList);
