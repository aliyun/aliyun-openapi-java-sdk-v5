...
public class SaveBatchTaskForCreatingOrderRenewRequest extends RpcAcsRequest<SaveBatchTaskForCreatingOrderRenewResponse> {
	...
	private List<OrderRenewParam> orderRenewParam;
	...
	public List<OrderRenewParam> getOrderRenewParam() {
		return this.orderRenewParam;
	}

	public void setOrderRenewParam(List<OrderRenewParam> orderRenewParam) {
		this.orderRenewParam = orderRenewParam;	
		if (orderRenewParam != null) {
			for (int depth1 = 0; depth1 < orderRenewParam.size(); depth1++) {
				putQueryParameter("OrderRenewParam." + (depth1 + 1) + ".SubscriptionDuration" , orderRenewParam.get(depth1).getSubscriptionDuration());
				putQueryParameter("OrderRenewParam." + (depth1 + 1) + ".CurrentExpirationDate" , orderRenewParam.get(depth1).getCurrentExpirationDate());
				putQueryParameter("OrderRenewParam." + (depth1 + 1) + ".DomainName" , orderRenewParam.get(depth1).getDomainName());
			}
		}	
	}
