...
public class SaveBatchTaskForModifyingDomainDnsRequest extends RpcAcsRequest<SaveBatchTaskForModifyingDomainDnsResponse> {
	...
	private List<String> domainNameServer;
	...
	public List<String> getDomainNameServer() {
		return this.domainNameServer;
	}

	public void setDomainNameServer(List<String> domainNameServer) {
		this.domainNameServer = domainNameServer;	
		if (domainNameServer != null) {
			for (int i = 0; i < domainNameServer.size(); i++) {
				putQueryParameter("DomainNameServer." + (i + 1) , domainNameServer.get(i));
			}
		}	
	}
