...
import com.aliyuncs.v5.rds.model.v20140815.DescribeReadDBInstanceDelayResponse;
import com.aliyuncs.v5.rds.model.v20140815.DescribeReadDBInstanceDelayResponse.ItemsItem;
import com.aliyuncs.v5.rds.model.v20140815.DescribeReadDBInstanceDelayResponse.ItemsItem.ReadonlyInstanceDelayItem;
...
public class DescribeReadDBInstanceDelayResponseUnmarshaller {
	...
	public static DescribeReadDBInstanceDelayResponse unmarshall(DescribeReadDBInstanceDelayResponse describeReadDBInstanceDelayResponse, UnmarshallerContext _ctx) {
		...
		for (int i = 0; i < _ctx.lengthValue("DescribeReadDBInstanceDelayResponse.Items.Length"); i++) {
			...
			List<ReadonlyInstanceDelayItem> readonlyInstanceDelayList = new ArrayList<ReadonlyInstanceDelayItem>();
			for (int j = 0; j < _ctx.lengthValue("DescribeReadDBInstanceDelayResponse.Items["+ i +"].ReadonlyInstanceDelay.Length"); j++) {
				...
				readonlyInstanceDelayList.add(readonlyInstanceDelayItem);
			}
			itemsItem.setReadonlyInstanceDelay(readonlyInstanceDelayList);

			itemsList.add(itemsItem);
		}
		describeReadDBInstanceDelayResponse.setItems(itemsList);
