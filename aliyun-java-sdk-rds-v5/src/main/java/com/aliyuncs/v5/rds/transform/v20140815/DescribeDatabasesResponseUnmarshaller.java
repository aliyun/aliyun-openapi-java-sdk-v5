...
import com.aliyuncs.v5.rds.model.v20140815.DescribeDatabasesResponse;
import com.aliyuncs.v5.rds.model.v20140815.DescribeDatabasesResponse.Database;
import com.aliyuncs.v5.rds.model.v20140815.DescribeDatabasesResponse.Database.AccountPrivilegeInfo;
...
public class DescribeDatabasesResponseUnmarshaller {
	...
	public static DescribeDatabasesResponse unmarshall(DescribeDatabasesResponse describeDatabasesResponse, UnmarshallerContext _ctx) {
		...
		List<Database> databasesList = new ArrayList<Database>();
		for (int i = 0; i < _ctx.lengthValue("DescribeDatabasesResponse.Databases.Length"); i++) {
			...
			List<AccountPrivilegeInfo> accountsList = new ArrayList<AccountPrivilegeInfo>();
			for (int j = 0; j < _ctx.lengthValue("DescribeDatabasesResponse.Databases["+ i +"].Accounts.Length"); j++) {
				...
				accountsList.add(accountPrivilegeInfo);
			}
			database.setAccounts(accountsList);

			databasesList.add(database);
		}
		describeDatabasesResponse.setDatabases(databasesList);
