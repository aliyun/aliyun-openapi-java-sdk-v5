...
public class ApplyPhysicalConnectionLOARequest extends RpcAcsRequest<ApplyPhysicalConnectionLOAResponse> {
	...
	public void setPMInfo(List<PMInfo> pMInfo) {
		this.pMInfo = pMInfo;	
		if (pMInfo != null) {
			for (int depth1 = 0; depth1 < pMInfo.size(); depth1++) {
				putQueryParameter("PMInfo." + (depth1 + 1) + ".PMCertificateNo" , pMInfo.get(depth1).getPMCertificateNo());
				putQueryParameter("PMInfo." + (depth1 + 1) + ".PMName" , pMInfo.get(depth1).getPMName());
				putQueryParameter("PMInfo." + (depth1 + 1) + ".PMCertificateType" , pMInfo.get(depth1).getPMCertificateType());
				putQueryParameter("PMInfo." + (depth1 + 1) + ".PMContactInfo" , pMInfo.get(depth1).getPMContactInfo());
				putQueryParameter("PMInfo." + (depth1 + 1) + ".PMGender" , pMInfo.get(depth1).getPMGender());
			}
		}	
	}
