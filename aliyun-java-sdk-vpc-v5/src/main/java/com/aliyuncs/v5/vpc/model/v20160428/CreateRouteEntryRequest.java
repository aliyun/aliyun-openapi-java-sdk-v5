...
public class CreateRouteEntryRequest extends RpcAcsRequest<CreateRouteEntryResponse> {
	...
	public void setNextHopList(List<NextHopList> nextHopList) {
		this.nextHopList = nextHopList;	
		if (nextHopList != null) {
			for (int depth1 = 0; depth1 < nextHopList.size(); depth1++) {
				putQueryParameter("NextHopList." + (depth1 + 1) + ".Weight" , nextHopList.get(depth1).getWeight());
				putQueryParameter("NextHopList." + (depth1 + 1) + ".NextHopId" , nextHopList.get(depth1).getNextHopId());
				putQueryParameter("NextHopList." + (depth1 + 1) + ".NextHopType" , nextHopList.get(depth1).getNextHopType());
			}
		}	
	}
