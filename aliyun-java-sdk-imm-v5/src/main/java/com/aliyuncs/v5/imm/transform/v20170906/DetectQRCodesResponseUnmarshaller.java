...
import com.aliyuncs.v5.imm.model.v20170906.DetectQRCodesResponse;
import com.aliyuncs.v5.imm.model.v20170906.DetectQRCodesResponse.FailDetailsItem;
import com.aliyuncs.v5.imm.model.v20170906.DetectQRCodesResponse.SuccessDetailsItem;
import com.aliyuncs.v5.imm.model.v20170906.DetectQRCodesResponse.SuccessDetailsItem.QRCodesItem;
import com.aliyuncs.v5.imm.model.v20170906.DetectQRCodesResponse.SuccessDetailsItem.QRCodesItem.QRCodesRectangle;
...
public class DetectQRCodesResponseUnmarshaller {
	...
	public static DetectQRCodesResponse unmarshall(DetectQRCodesResponse detectQRCodesResponse, UnmarshallerContext _ctx) {
		...
		List<SuccessDetailsItem> successDetailsList = new ArrayList<SuccessDetailsItem>();
		for (int i = 0; i < _ctx.lengthValue("DetectQRCodesResponse.SuccessDetails.Length"); i++) {
			...
			List<QRCodesItem> qRCodesList = new ArrayList<QRCodesItem>();
			for (int j = 0; j < _ctx.lengthValue("DetectQRCodesResponse.SuccessDetails["+ i +"].QRCodes.Length"); j++) {
				...
				qRCodesList.add(qRCodesItem);
			}
			successDetailsItem.setQRCodes(qRCodesList);

			successDetailsList.add(successDetailsItem);
		}
		detectQRCodesResponse.setSuccessDetails(successDetailsList);

		List<FailDetailsItem> failDetailsList = new ArrayList<FailDetailsItem>();
		for (int i = 0; i < _ctx.lengthValue("DetectQRCodesResponse.FailDetails.Length"); i++) {
			...
			failDetailsList.add(failDetailsItem);
		}
		detectQRCodesResponse.setFailDetails(failDetailsList);
