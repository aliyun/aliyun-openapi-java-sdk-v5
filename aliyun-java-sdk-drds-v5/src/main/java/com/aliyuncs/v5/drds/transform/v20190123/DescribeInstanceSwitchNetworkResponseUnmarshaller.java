...
import com.aliyuncs.v5.drds.model.v20190123.DescribeInstanceSwitchNetworkResponse;
import com.aliyuncs.v5.drds.model.v20190123.DescribeInstanceSwitchNetworkResponse.VpcInfo;
import com.aliyuncs.v5.drds.model.v20190123.DescribeInstanceSwitchNetworkResponse.VpcInfo.VswitchInfo;
...
public class DescribeInstanceSwitchNetworkResponseUnmarshaller {
	...
	public static DescribeInstanceSwitchNetworkResponse unmarshall(DescribeInstanceSwitchNetworkResponse describeInstanceSwitchNetworkResponse, UnmarshallerContext _ctx) {
		...
		List<VpcInfo> vpcInfosList = new ArrayList<VpcInfo>();
		for (int i = 0; i < _ctx.lengthValue("DescribeInstanceSwitchNetworkResponse.VpcInfos.Length"); i++) {
			...
			List<VswitchInfo> vswitchInfosList = new ArrayList<VswitchInfo>();
			for (int j = 0; j < _ctx.lengthValue("DescribeInstanceSwitchNetworkResponse.VpcInfos["+ i +"].VswitchInfos.Length"); j++) {
				...
				vswitchInfosList.add(vswitchInfo);
			}
			vpcInfo.setVswitchInfos(vswitchInfosList);

			vpcInfosList.add(vpcInfo);
		}
		describeInstanceSwitchNetworkResponse.setVpcInfos(vpcInfosList);
