...
public class CreateMyCatEvaluateRequest extends RpcAcsRequest<CreateMyCatEvaluateResponse> {
	...
	private List<String> importDbList;
	...
	public List<String> getImportDbList() {
		return this.importDbList;
	}

	public void setImportDbList(List<String> importDbList) {
		this.importDbList = importDbList;	
		if (importDbList != null) {
			for (int i = 0; i < importDbList.size(); i++) {
				putQueryParameter("ImportDbList." + (i + 1) , importDbList.get(i));
			}
		}	
	}
