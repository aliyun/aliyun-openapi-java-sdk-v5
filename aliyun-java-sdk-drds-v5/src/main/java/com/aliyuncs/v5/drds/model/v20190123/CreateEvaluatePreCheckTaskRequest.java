...
public class CreateEvaluatePreCheckTaskRequest extends RpcAcsRequest<CreateEvaluatePreCheckTaskResponse> {
	...
	public void setDbInfo(List<DbInfo> dbInfo) {
		this.dbInfo = dbInfo;	
		if (dbInfo != null) {
			for (int depth1 = 0; depth1 < dbInfo.size(); depth1++) {
				putQueryParameter("DbInfo." + (depth1 + 1) + ".InstId" , dbInfo.get(depth1).getInstId());
				putQueryParameter("DbInfo." + (depth1 + 1) + ".DbPort" , dbInfo.get(depth1).getDbPort());
				putQueryParameter("DbInfo." + (depth1 + 1) + ".DbName" , dbInfo.get(depth1).getDbName());
				putQueryParameter("DbInfo." + (depth1 + 1) + ".DbPassword" , dbInfo.get(depth1).getDbPassword());
				putQueryParameter("DbInfo." + (depth1 + 1) + ".DbUser" , dbInfo.get(depth1).getDbUser());
			}
		}	
	}
