...
import com.aliyuncs.v5.waf_openapi.model.v20190910.DescribeDomainResponse;
import com.aliyuncs.v5.waf_openapi.model.v20190910.DescribeDomainResponse.Domain;
import com.aliyuncs.v5.waf_openapi.model.v20190910.DescribeDomainResponse.Domain.CloudNativeInstancesItem;
import com.aliyuncs.v5.waf_openapi.model.v20190910.DescribeDomainResponse.Domain.CloudNativeInstancesItem.ProtocolPortConfigsItem;
import com.aliyuncs.v5.waf_openapi.model.v20190910.DescribeDomainResponse.Domain.LogHeader;
...
public class DescribeDomainResponseUnmarshaller {
	...
	public static DescribeDomainResponse unmarshall(DescribeDomainResponse describeDomainResponse, UnmarshallerContext _ctx) {
		...
		List<LogHeader> logHeadersList = new ArrayList<LogHeader>();
		for (int i = 0; i < _ctx.lengthValue("DescribeDomainResponse.Domain.LogHeaders.Length"); i++) {
			...
			logHeadersList.add(logHeader);
		}
		domain.setLogHeaders(logHeadersList);

		List<CloudNativeInstancesItem> cloudNativeInstancesList = new ArrayList<CloudNativeInstancesItem>();
		for (int i = 0; i < _ctx.lengthValue("DescribeDomainResponse.Domain.CloudNativeInstances.Length"); i++) {
			...
			List<ProtocolPortConfigsItem> protocolPortConfigsList = new ArrayList<ProtocolPortConfigsItem>();
			for (int j = 0; j < _ctx.lengthValue("DescribeDomainResponse.Domain.CloudNativeInstances["+ i +"].ProtocolPortConfigs.Length"); j++) {
				...
				protocolPortConfigsList.add(protocolPortConfigsItem);
			}
			cloudNativeInstancesItem.setProtocolPortConfigs(protocolPortConfigsList);

			cloudNativeInstancesList.add(cloudNativeInstancesItem);
		}
		domain.setCloudNativeInstances(cloudNativeInstancesList);
