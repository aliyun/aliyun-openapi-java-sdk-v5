...
import com.aliyuncs.v5.market.model.v20151101.QueryMarketCategoriesResponse;
import com.aliyuncs.v5.market.model.v20151101.QueryMarketCategoriesResponse.Category;
import com.aliyuncs.v5.market.model.v20151101.QueryMarketCategoriesResponse.Category.ChildCategory;
...
public class QueryMarketCategoriesResponseUnmarshaller {
	...
	public static QueryMarketCategoriesResponse unmarshall(QueryMarketCategoriesResponse queryMarketCategoriesResponse, UnmarshallerContext _ctx) {
		...
		for (int i = 0; i < _ctx.lengthValue("QueryMarketCategoriesResponse.Categories.Length"); i++) {
			...
			List<ChildCategory> childCategoriesList = new ArrayList<ChildCategory>();
			for (int j = 0; j < _ctx.lengthValue("QueryMarketCategoriesResponse.Categories["+ i +"].ChildCategories.Length"); j++) {
				...
				childCategoriesList.add(childCategory);
			}
			category.setChildCategories(childCategoriesList);

			categoriesList.add(category);
		}
		queryMarketCategoriesResponse.setCategories(categoriesList);
