...
public class BatchAddDeviceGroupRelationsRequest extends RpcAcsRequest<BatchAddDeviceGroupRelationsResponse> {
	...
	public void setDevice(List<Device> device) {
		this.device = device;	
		if (device != null) {
			for (int depth1 = 0; depth1 < device.size(); depth1++) {
				putQueryParameter("Device." + (depth1 + 1) + ".DeviceName" , device.get(depth1).getDeviceName());
				putQueryParameter("Device." + (depth1 + 1) + ".ProductKey" , device.get(depth1).getProductKey());
			}
		}	
	}
