	...
	public void setConfigs(List<Configs> configs) {
		...
		if (configs != null) {
			for (int depth1 = 0; depth1 < configs.size(); depth1++) {
				putQueryParameter("Configs." + (depth1 + 1) + ".Format" , configs.get(depth1).getFormat());
				putQueryParameter("Configs." + (depth1 + 1) + ".Content" , configs.get(depth1).getContent());
				putQueryParameter("Configs." + (depth1 + 1) + ".Key" , configs.get(depth1).getKey());
			}
		}	
	}
