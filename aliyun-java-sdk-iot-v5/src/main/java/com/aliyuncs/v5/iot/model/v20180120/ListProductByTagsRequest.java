...
public class ListProductByTagsRequest extends RpcAcsRequest<ListProductByTagsResponse> {
	...
	public void setProductTag(List<ProductTag> productTag) {
		...
		if (productTag != null) {
			for (int depth1 = 0; depth1 < productTag.size(); depth1++) {
				putQueryParameter("ProductTag." + (depth1 + 1) + ".TagValue" , productTag.get(depth1).getTagValue());
				putQueryParameter("ProductTag." + (depth1 + 1) + ".TagKey" , productTag.get(depth1).getTagKey());
			}
		}	
	}
