...
import com.aliyuncs.v5.iot.model.v20180120.GetSpeechVoiceResponse;
import com.aliyuncs.v5.iot.model.v20180120.GetSpeechVoiceResponse.Items;
import com.aliyuncs.v5.iot.model.v20180120.GetSpeechVoiceResponse.Items.Items1;
...
public class GetSpeechVoiceResponseUnmarshaller {
	...
	public static GetSpeechVoiceResponse unmarshall(GetSpeechVoiceResponse getSpeechVoiceResponse, UnmarshallerContext _ctx) {
		...
		for (int i = 0; i < _ctx.lengthValue("GetSpeechVoiceResponse.Data.Length"); i++) {
			...
			List<Items1> voiceListList = new ArrayList<Items1>();
			for (int j = 0; j < _ctx.lengthValue("GetSpeechVoiceResponse.Data["+ i +"].VoiceList.Length"); j++) {
				...
				voiceListList.add(items1);
			}
			items.setVoiceList(voiceListList);

			dataList.add(items);
		}
		getSpeechVoiceResponse.setData(dataList);
