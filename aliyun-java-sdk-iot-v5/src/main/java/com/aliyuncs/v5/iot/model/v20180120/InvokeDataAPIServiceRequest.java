...
public class InvokeDataAPIServiceRequest extends RpcAcsRequest<InvokeDataAPIServiceResponse> {
	...
	public void setParam(List<Param> param) {
		this.param = param;	
		if (param != null) {
			for (int depth1 = 0; depth1 < param.size(); depth1++) {
				putBodyParameter("Param." + (depth1 + 1) + ".ParamType" , param.get(depth1).getParamType());
				if (param.get(depth1).getListParamValue() != null) {
					for (int i = 0; i < param.get(depth1).getListParamValue().size(); i++) {
						putBodyParameter("Param." + (depth1 + 1) + ".ListParamValue." + (i + 1) , param.get(depth1).getListParamValue().get(i));
					}
				}
				putBodyParameter("Param." + (depth1 + 1) + ".ListParamType" , param.get(depth1).getListParamType());
				putBodyParameter("Param." + (depth1 + 1) + ".ParamName" , param.get(depth1).getParamName());
				putBodyParameter("Param." + (depth1 + 1) + ".ParamValue" , param.get(depth1).getParamValue());
			}
		}	
	}
	...
	public static class Param {
		...
		public void setListParamValue(List<String> listParamValue) {
