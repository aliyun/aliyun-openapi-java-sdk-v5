...
public class ListAnalyticsDataRequest extends RpcAcsRequest<ListAnalyticsDataResponse> {
	...
	public void setCondition(List<Condition> condition) {
		...
		if (condition != null) {
			for (int depth1 = 0; depth1 < condition.size(); depth1++) {
				putQueryParameter("Condition." + (depth1 + 1) + ".FieldName" , condition.get(depth1).getFieldName());
				putQueryParameter("Condition." + (depth1 + 1) + ".Operate" , condition.get(depth1).getOperate());
				putQueryParameter("Condition." + (depth1 + 1) + ".BetweenStart" , condition.get(depth1).getBetweenStart());
				putQueryParameter("Condition." + (depth1 + 1) + ".BetweenEnd" , condition.get(depth1).getBetweenEnd());
				putQueryParameter("Condition." + (depth1 + 1) + ".Value" , condition.get(depth1).getValue());
			}
		}	
	}
